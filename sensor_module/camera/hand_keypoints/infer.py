# refer:  http://blog.handcare.org/blog/2017/10/26/anatomy-101-finger-joints/
import platform
...
class HandDetect():
    ...
    def run(self,img=None,video=None):
        ...
        while True:
            ...
            if self.frame_count % 7 == 0:
                ...
                kp, box, _ = self.detector(color_image, return_track, False)
            if kp is not None and box is not None:
                ...
                if len(gasture_pre) > 0:
                    ...
                    cv2.putText(show_image,
                                'rank 1 pre  %s probably %f' % (self.reverse_gasture_define[max_index], gasture_pre[0][1]), (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 155), 1, cv2.LINE_AA)
